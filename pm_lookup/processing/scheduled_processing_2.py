...
def arrange_datapoints_series_and_graphs():
    ...
    for set_of_serie_parameters in sets_of_serie_parameters:
        ...
        area_di_interesse = set_of_serie_parameters.target_area
        time_horizon = set_of_serie_parameters.time_horizon
        ...
        records_serie_storica = HistoricalDatapoints.objects.filter(
            ...
            Last_update_time__gte = timezone.now() - time_horizon,
            Last_update_time__lte = timezone.now()
        )

        # nota: i dati sono già ordinati per default in ordine decrescente
        ...
        # aggiunto per fixare il fatto che nei grafici è mostrato orario come se fosse in UTC
        # errore sopraggiunto dopo il reset del db?
        serie_storica["last_update_time_values"] = fix_timezone_mismatch_in_array_of_datetimes(serie_storica["last_update_time_values"])


        # group elements of the serie into group of data 
        # that must have their values turned to a mean or other statistics
        #---------------------------------------------------------------------

        # scorrere time horizon per aggregation period, e continuare il cliclo finchè il rimasto di time horizon non è <=0
