...
from pm_lookup.models import HourlyAggregatedDatapointsSerie
...
def arrange_hourlyaggregated_datapoints_series_and_graphs():
    ...
    print("Eliminate tutte le serie storiche orarie in HourlyAggregatedDatapointsSerie!")
    ...
    for area_di_interesse in TargetArea.objects.all():
        ...
        # prendo i record delle ultime 24 ore
        # aggiornamento unitùà temporali al default
        n_ore = 24
      
        time_horizon_length = n_ore
        ...
        # da raggiungere i ore dichiarati per la lunghezza della serie storica oraria 
        # ( per default ho messo 30 ore),  
        # allora Bisogna porre il numero di ore uguale 
        ...
        if n_HistoricalDatapoints_records < time_horizon_length:
            ...
            print("Non ci sono dati sufficienti per realizzare la serie storica di almeno %s ore." % n_ore)
            
            # aggiornamento unità temporali
            n_ore = int (math.floor( n_HistoricalDatapoints_records / n_ore ) )
            time_horizon_length = n_ore * n_ore
            ...
            # Allora Bisogna porre il numero di ore pari ad uno e 
            # il numero di ore pari 
            # alla quantità di dati presenti all'interno del modello storico orario
            if n_ore == 0:
            ...
            print("Viene mostrata una serie storica lunga solo %s ore." % n_ore)
        ...
        # records_serie_storica = [  round( np.mean( records_serie_storica[ 0 + 24*i : 24 + 24*i] ) , 2)  for i in range(n_ore)]
        ...
        PM10_hourly_mean_quality_cathegory_label = [ evaluate_PM10(i)[0] for i in PM10_hourly_mean ]
        PM25_hourly_mean_quality_cathegory_label = [ evaluate_PM25(i)[0] for i in PM25_hourly_mean ]
        ...
        serie_storica = {
                        ...
                        "PM10_hourly_mean_quality_cathegory_label" : PM10_hourly_mean_quality_cathegory_label,
                        "PM25_hourly_mean_quality_cathegory_label" : PM25_hourly_mean_quality_cathegory_label,
        ...
        elementi_grafico = HourlyAggregatedDatapointsSerie(
                                                    # errore qui
                                                    ...
                                                    PM10_mean_quality_cathegory_label_values = '["' + '", "'.join(str(e) for e in  serie_storica['PM10_hourly_mean_quality_cathegory_label'] ) +'"]',
                                                    PM25_mean_quality_cathegory_label_values = '["' + '", "'.join(str(e) for e in  serie_storica['PM25_hourly_mean_quality_cathegory_label'] ) +'"]',

                                                    PM10_mean_quality_cathegory_values = '["' + '", "'.join(str(e) for e in  serie_storica['PM10_hourly_mean_cathegory'] ) +'"]',
